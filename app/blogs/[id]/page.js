...
export default function BlogPage() {
	...
	const handleClap = async () => {
		const res = await fetch("/api/blogs/claps", {
			...
			body: JSON.stringify({ blogId: id, userId: "user-123" })
		});
	...
	const handleCommentSubmit = async (e) => {
		...
		const res = await fetch("/api/blogs/comments", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ blogId: id, userId: "user-123", text: newComment })
		});
		const data = await res.json();
		if (data.success) setComments(prev => [...prev, data.comment]);
		setNewComment("");
	};
					...
					<div className="space-y-4">
						{comments.map(c => (
							<div key={c.comment_id} className="border p-3 rounded bg-gray-50">
								<p className="font-medium">{c.user_id || "Anonymous"}</p>
								<p>{c.text}</p>
							</div>
						))}
					</div>
